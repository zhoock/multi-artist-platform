...
export function buildPublicAlbumPath(albumId: string, artistSlug: string): string {
  return withPublicArtistQuery(`/albums/${albumId.trim()}`, artistSlug.trim());
}
 ...
 *
 * - Direct album route → replace `/albums/:newId?artist=`
...
export function navigateAfterAlbumSlugRename(input: {
  ...
  const nextPath = buildPublicAlbumPath(newAlbumId, artistSlug);
