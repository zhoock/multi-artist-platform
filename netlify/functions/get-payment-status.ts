...
async function buildDevPaymentStatusFromDb(
  resolvedOrderId: string,
  paymentId: string
  ...
  const paymentRow = await query<{
    ...
    `SELECT provider_payment_id, status, amount, currency, raw_last_event
     FROM payments
     WHERE order_id = $1 AND provider = 'yookassa' AND provider_payment_id = $2
     LIMIT 1`,
    [resolvedOrderId, paymentId]
  );

  const row = paymentRow.rows[0];
  if (!row || !isDevMarkedPayment(row.raw_last_event)) {
    return null;
  }

  const orderMeta = await query<{ album_id: string; customer_email: string }>(
    'SELECT album_id, customer_email FROM orders WHERE id = $1',
    [resolvedOrderId]
  );
  ...
  try {
    ...
    if (isDevPaymentModeEnabled()) {
      ...
      if (devPaymentStatus) {
        ...
        const orderUpdated = await applyAlbumPaymentSuccess(devPaymentStatus);
        const album = await resolvePurchasedAlbumForResponse(
          resolvedOrderId,
          devPaymentStatus.metadata
        );
        ...
        return {
          ...
          body: JSON.stringify({
            success: true,
            payment: {
              id: devPaymentStatus.id,
              status: devPaymentStatus.status,
              paid: devPaymentStatus.paid,
              amount: devPaymentStatus.amount,
              metadata: devPaymentStatus.metadata,
            },
            orderUpdated,
            ...(album ? { album } : {}),
          } as PaymentStatusResponse),
